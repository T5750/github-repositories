...
public class PermissionSubject {
	private static final Logger LOGGER = LoggerFactory
			.getLogger(PermissionSubject.class);
	/**
	...
	public void update(String appCode) {
		for (String code : sets) {
			if (code.equals(appCode)) {
				try {
					permissionJmsService.send(ConfigUtils
							.getProperty("mq.permission.queue.prefix")
							.concat(appCode), appCode);
	...
	public void update() {
		for (String appCode : sets) {
			try {
				permissionJmsService.send(
						ConfigUtils.getProperty("mq.permission.queue.prefix")
								.concat(appCode),
						appCode);
				LOGGER.info("消息服务通知appCode为：{}的应用更新权限", appCode);
			} catch (Exception e) {
