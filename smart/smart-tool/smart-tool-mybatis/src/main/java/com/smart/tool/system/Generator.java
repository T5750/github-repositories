...
import com.smart.tool.generate.*;
...
public class Generator extends BaseFrame {
	...
	public static void main(String[] args) {
		...
		editor.setLocation((screenSize.width - ownerSize.width) / 2,
				(screenSize.height - ownerSize.height) / 2);
		editor.setVisible(true);
	}

	protected void changeTextValue() {
		this.modelText.setText(getPackageName() + "model" + (analyzer != null
				? "/" + analyzer.getModelName() + ".java" : ""));
		this.serviceText
				.setText(getPackageName() + "service" + (analyzer != null
						? "/" + analyzer.getModelName() + "Service.java" : ""));
		this.ServiceImplText.setText(getPackageName() + "service/impl"
				+ (analyzer != null
						? "/" + analyzer.getModelName() + "ServiceImpl.java"
						: ""));
		this.daoText.setText(getPackageName() + "dao" + (analyzer != null
				? "/" + analyzer.getModelName() + "Dao.java" : ""));
		this.modelXmlText
				.setText(getPackageName() + "mapper" + (analyzer != null
						? "/" + analyzer.getModelName() + ".xml" : ""));
		String adminString = (adminCheckBox.isSelected() ? "/" + ADMIN : "");
		this.controllerText.setText(getPackageName() + "controller"
				+ adminString
				+ (analyzer != null
						? "/" + analyzer.getModelName() + "Controller.java"
						: ""));
		this.listText
				.setText(getViewBasePath() + adminString + (analyzer != null
						? "/" + analyzer.getLowerModelName() + ".jsp" : ""));
		this.editText.setText(getViewBasePath() + adminString
				+ (analyzer != null
						? "/" + analyzer.getLowerModelName() + "Edit.jsp"
						: ""));
	}
	...
	private String getPackageName() {
		return getClassBasePath() + "/" + "com" + "/" + config.getCompanyName()
				+ "/"
				+ (StringUtils.isNotBlank(projectText.getText())
						? projectText.getText() + "/" : "")
				+ (StringUtils.isNotBlank(moduleText.getText())
						? moduleText.getText() + "/" : "");
	}
	...
	private void generateModelFile(String basePath) {
		...
		Set<String> excludeFieldSet = persistentMap
				.get(extendsBox.getSelectedItem());
		...
		FileUtils.createFile(basePath, modelText.getText(),
				new Model(config.getCompanyName(), projectText.getText(),
						moduleText.getText(), analyzer.getModelName(),
						fieldList, analyzer.getTableName(),
						extendsBox.getSelectedItem().toString(),
						Long.valueOf(StringUtils.getRandom(17)).toString(),
						analyzer.isContainEnable(), containDate,
	...
	private void generateModelXmlFile(String basePath) {
		...
		FileUtils.createFile(basePath, modelXmlText.getText(),
				new ModelXml(config.getCompanyName(), projectText.getText(),
						moduleText.getText(), analyzer.getModelName(),
						fieldList, analyzer.getTableName(),
						extendsBox.getSelectedItem().toString(),
						Long.valueOf(StringUtils.getRandom(17)).toString(),
	...
	private void generateServiceFile(String basePath) {
		FileUtils.createFile(basePath, serviceText.getText(),
				new Service(config.getCompanyName(), projectText.getText(),
						moduleText.getText(), analyzer.getModelName())
								.getHtml());
	}
	...
	private void generateServiceImplFile(String basePath) {
		FileUtils.createFile(basePath, ServiceImplText.getText(),
				new ServiceImpl(config.getCompanyName(), projectText.getText(),
						moduleText.getText(), analyzer.getModelName())
								.getHtml());
	}

	private void generateDaoFile(String basePath) {
		FileUtils.createFile(basePath, daoText.getText(),
				new Dao(config.getCompanyName(), projectText.getText(),
						moduleText.getText(), analyzer.getModelName())
								.getHtml());
	}
	...
	private void generateControllerFile(String basePath) {
		...
		FileUtils.createFile(basePath, controllerText.getText(),
				new Controller(config.getCompanyName(), projectText.getText(),
						moduleText.getText(), analyzer.getModelName(),
						fieldList, analyzer.isContainEnable(), containDate,
						analyzer.getTableComment(),
						adminCheckBox.isSelected() ? ADMIN : null).getHtml());
	}
	...
	private void generateListFile(String basePath) {
		if (StringUtils.isNotBlank(this.listText.getText())) {
			...
			FileUtils.createFile(basePath, listText.getText(),
					new com.smart.tool.generate.List(analyzer.getTableComment(),
							analyzer.getModelName(), analyzer.isContainEnable(),
							Analyzer.ENABLE_NAME, fieldList).getHtml());
		}
	}
	...
	private void generateEditFile(String basePath) {
		if (StringUtils.isNotBlank(this.editText.getText())) {
			...
			FileUtils.createFile(basePath, editText.getText(),
					new Edit(analyzer.getTableComment(),
							analyzer.getModelName(), analyzer.isContainEnable(),
							fieldList).getHtml());
		}
	}
