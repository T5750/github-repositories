...
public class ClazzBizImpl implements ClazzBiz {
	...
	public void add(String deptName, String specName, String year) {
		...
		clazz.setClazzId(year.substring(2) + deptDao.findIdByName(deptName)
				+ specDao.findIdByName(specName) + classCount);
		clazz.setYear(year);
		clazzDao.add(clazz);
	}
	...
	// TODO
	// 在这里感觉不应该查询两次应该使用上面的结果keyset,磁盘的开销远大于运算,如果开一个deptAndSpecList变量存储这个变量是线程安全的么,留坑=
	// =！
