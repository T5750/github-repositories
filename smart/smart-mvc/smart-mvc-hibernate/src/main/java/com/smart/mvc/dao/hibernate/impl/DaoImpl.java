...
public abstract class DaoImpl<T, ID extends Serializable>
	 ...
	 * @return
	 */
	protected List<T> findByHql(String hql, boolean cacheAble,
			Object... values) {
	 ...
	 * @return
	 */
	protected T findByHqlFirst(String hql, boolean cacheAble,
			Object... values) {
	 ...
	 * @return
	 */
	protected List<T> findBySql(String sql, boolean cacheAble,
			Object... values) {
	 ...
	 * @return
	 */
	protected T findBySqlFirst(String sql, boolean cacheAble,
			Object... values) {
		return (T) createSqlQuery(sql, entityClass, cacheAble, values)
				.uniqueResult();
	}
	...
	protected List<T> pageByRecursionSql(String sql, boolean cacheAble,
		...
		// 查询当前页记录
		SQLQuery query = createSqlQuery(querySql, entityClass, cacheAble,
				values);
		query.setFirstResult(p.getFirstResult());
		query.setMaxResults(p.getPageSize());
		p.setList(query.list());
	 ...
	 *            要查询的属性列表
	 * @param sortHql
	 *            排序语句 例如：sort DESC,createTime DESC
	 * @param values
	 *            要匹配的属性值列表
	 * @return
	 */
	@Override
	public List<T> findByAll(Pagination<T> p, String[] property, String sortHql,
			Object... values) {
	...
	private StringBuffer generateHql(String[] property, Object... values) {
		if (StringUtils.isBlank(property) || values == null) {
			return new StringBuffer(
					" from " + entityClass.getSimpleName() + " t ");
		}
		...
		StringBuffer hql = new StringBuffer(
				" from " + entityClass.getSimpleName() + " t where 1=1 ");
			...
			if (StringUtils.isNotBlank(s)) {
				if (values[i].getClass().isArray()) {
					...
					for (int j = 0, size = valuesArrayItem.length; j < size; j++) {
						if (j == 0) {
							hql.append(" t.").append(property[i])
									.append(" =? ");
						} else {
							hql.append(" or t.").append(property[i])
									.append(" =? ");
						}
					}
					hql.append(" )");
				...
				throw new RuntimeException(
						"在" + entityClass.getName() + "中不存在属性:" + cachePro[i]);
			}
		}
		return hql;
	}
	...
	private String filterName(String filterText) {
		String[] list = { "and", "exec", "insert", "select", "delete", "update",
	 ...
	 * 创建Hql查询语句
	 */
	private Query createHqlQuery(String hql, boolean cacheAble,
			Object... values) {
	 ...
	 * 创建Sql查询语句
	 */
	private SQLQuery createSqlQuery(String sql, Class entityClass,
			boolean cacheAble, Object... values) {
