...
public abstract class BaseFrame extends JFrame {
	...
	public BaseFrame() {
		...
		config = new DbConfig();
		// 创建数据库连接
		connection = DbManager.getConnection(config.getDriverClassName(),
	...
	private void init() {
		...
		this.tablesBox = new JComboBox(
				DbManager.getTableNameList(connection).toArray());
		...
		this.tablesBox.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				analyzer = new Analyzer(statement, ((JComboBox) e.getSource())
						.getSelectedItem().toString(), config.getUrl());
				changeTextValue();
			}
		});
